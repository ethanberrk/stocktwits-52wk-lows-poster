"""52-week-LOW source on Xignite (Ethan's licensed subscription).

Universe: Nasdaq Trader symbol directories (Nasdaq + NYSE/NYSE American,
          keyless, official, ETFs and test issues excluded).
Quotes:   GlobalQuotes delayed — Low/Low52Weeks/Last/%chg/Volume/exchange/name.
...
Keep:     traded today, exchange in NYSE/NASDAQ/AMEX, common equity by
          name+symbol shape, $5M dollar-volume floor, market cap >= $1B
          (same floor the Yahoo screen applied, so select.validate's
          plausibility gate means the same thing).
"""
import re
...
def build_candidate(ticker: str, listed_name: str, q: dict, mcap) -> Candidate | None:
    sec = q.get("Security") or {}
    exchange = sec.get("Market")
    if exchange not in config.XIGNITE_EXCHANGES:
        return None
    name = sec.get("Name") or listed_name
    if config.NAME_EXCLUDE_RE.search(name):
        return None
    if config.PREFERRED_RE.search(ticker) or config.WARRANT_RE.search(ticker):
        return None
    ...
    if float(last) * float(q.get("Volume") or 0.0) < config.MIN_DOLLAR_VOLUME:
        return None
    return Candidate(
        ticker=ticker,
        name=name,
